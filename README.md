...
| Communication protocol stack | 🔄 **Under revision — was marked complete, and wasn't.** Three separate design faults are now documented: it does not scale ([`docs/routing.md`](docs/routing.md) — twenty nodes on one segment spend 30.7% of the channel on routing advertisements), it has no authentication of any kind ([`docs/threat-model.md`](docs/threat-model.md)), and its channel model ranks the Wi-Fi mesh as a fallback for the powerline when the two reach different sets of nodes ([`docs/channels.md`](docs/channels.md)). Each has a decided design; none is implemented, and [`docs/protocol.md`](docs/protocol.md) still specifies the old behaviour alongside notices of what replaces it |
...
| Software architecture (Zephyr + Forth VM) | ⚠️ Complete on paper, with one known conflict — [`docs/threat-model.md`](docs/threat-model.md) found that Ed25519's 1–2KB working set does not fit the `ROUTER` task's 2048B stack, which is why the crypto moves to the ESP32-C3 and the node's identity ends up belonging to that module |
| Electrical safety analysis | ✅ Complete |
| Protocol & Forth VM reference prototypes ([`tools/`](tools/), Python, pre-hardware validation) | ✅ Complete — 90 tests pass. Note that `protocol-sim` implements the protocol as specified today, so the routing, authentication and channel revisions above will need to land there too before they mean anything |
...
| **Link-budget survey** (rides along with the prototype) | 📋 Blocking four documented decisions — cross-phase attenuation, how much of a real segment hears itself, mesh reach through actual walls, and ESP-NOW throughput for small frames. Every routing and channel figure in this project is arithmetic from datasheet-class assumptions, not evidence. This is now the cheapest thing that would tell us whether the design is right |
